...
def generate_ofdm_signal(n_sym=default_n_sym, coeff=None, nfft=fft_len, cp=cp_len, nsc=nsc):
    ...
    y_ofdm *= coeff
    return y_ofdm.astype(np.complex64), sQ, None

# def ofdm_demod(sig, coeff=None, nfft=fft_len, cp=cp_len, nsc=nsc):
#     if coeff is None:
#         coeff = normalizing_coeff
